...
The original problem I sought to address was to improve the accessibility of our codebase's `Select` menu component. This component had a few accessibility related issues with it, primarily not correctly implementing keyboard navigation and focus management. These issues stemmed partly from the fact that the component in question was built as a custom `Select` menu from the start and that it happened to be a wrapper component around another custom `DropDown` component. With component based UI libraries such as React.JS this can be a common technique for enhancing or extending a component. Because React encourages [composition over inheritance](#), a React developer may take an existing component and render it inside a new component with some additional features or behavioral modifications.

For example let's say you have a button component but you need variants of this component such as primary, secondary, and tertiary to match the design system your team has created, maintains, and continuously updates. Each of these button variants should have its own styling associated with it, and perhaps options for different sizes, states such as disabled or invalid, ability to be displayed with an icon, etc. Rather than having a single button component handle all of these style and potentially behavior patterns you may create a single "basic button" component that handles things like event handlers and attributes. Using the composition design pattern you would then create separate components that wrap this basic component to make it look and behave like a primary or secondary button.

While there's nothing wrong about this approach if it's done cautiously and thoughtfully, it can easily become abused if care and planning are not taken. Take for example the concept of a "dropdown" menu. The concept of something that "drops down" in User Interface land can mean a lot of different things. It could be a menu that "drops down" below a button in a navigation when clicked on. It might even be a select menu that opens by "dropping down" after a user clicks on it. Maybe a developer sees this component and thinks "I could use this for a dialog menu (e.g. a popup that appears when clicking on a button anywhere in the UI) because it already has the behavior I need". These all may sound like they're similar enough to use the same underlying base component, but they actually have different semantics and most importantly accessibility requirements. So what can become a problem is if the underlying component happens to be a popup UI element that allows for any sort of child elements in it in almost any kind of context. How do you correctly implement focus management? How do you handle keyboard navigation? What about adding the correct ARIA attributes? If care isn't taken this can easily lead to a lot of usability and accessibility problems in your website or app.
