...
After flushing out the software design doc and reviewing it with my team, I realized some uncertainty remained about a few key aspects of the Activity Map block. A significant source of this had to do with how the layout and concept would hold up when using real world data. While the design mocks for the feature showed the best case scenario of how the feature would look with one sample dataset I provided, I knew from my past experience doing data visualization work that there could be plenty of things that might break parts of the design. For a GPX file containing data of a very long activity such as a long distance bicycle race, we would need adequate space for the summary statistics to hold larger values. The elevation profile chart would also benefit from more horizontal room to show the variation in elevation if the activity traversed mountainous terrain.

This led me to develop several prototypes outside of our production codebase in the form of a small SvelteJS app, Codepens, and a small React app. The different choices in frontend tech might sound random or odd, but they were for good reason. For example, I find SvelteJS very easy to reason about and quick to work with compared to React, so I chose it for making a prototype of the Activity Map block to test real world data with. To solve some of our layout challenges which required various cells within the `<article>` container expanding and contracting their horizontal space, I created a Codepen that consisted of mostly HTML and CSS with a sprinkling of JS. When I discovered that the ArcGIS JavaScript SDK would not render a chart that was customizable to the extent that we needed it to be, I prototyped an area chart using D3JS and React that could be tested with real data.

Doing this work outside of the production codebase (which for the record consists primarily of ReactJS, NextJS, TypeScript, and CSS in JS) meant that I was not burdened by slow build times (our app is quite large, consisting of thousands of modules), type checking, automated tests, code reviews, and more generally the limitations of patterns we adhere to within our production code to keep our code standardized. I did not feel bothered by refactoring code from each of the prototypes when I felt parts of them could be used in our production codebase. If there's one thing I've noticed about becoming a more senior it's that I feel less precious about writing, removing, and refactoring code.
